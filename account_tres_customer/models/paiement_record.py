...
class PaiementCaisse(models.Model):
    ...
    def _calc_total_amount(self):
        ...
        self.nb_ov = len(self.ov_lines)
        self.nb_cb = len(self.cb_lines)
        self.total_amount = sum(cheque.amount for cheque in self.cheque_lines) + sum(effet.amount for effet in self.effet_lines)+ \
                            sum(ov.amount for ov in self.ov_lines) + sum(cash.amount for cash in self.cash_lines) + sum(cb.amount for cb in self.cb_lines)

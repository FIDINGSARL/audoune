...
class PrintAssetWizard(models.TransientModel):
    ...
    def print_action(self):
        ...
        assets = self.env['account.asset.asset'].read_group(domain=[], fields=['name', 'affectataire', 'code_inventaire'], groupby=['affectataire'])
        # print('asssets_dict', [item['affectataire'][0] for item in assets])
        # assets_dict = dict((item['name'], item['affectataire'], item['code_inventaire']) for item in assets)


        return self.env.ref('account_asset_extend.report_asset_location').with_context(discard_logo_check=True).report_action([], data=data)
